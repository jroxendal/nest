...
class FastAPIMiddlewareTestCase(unittest.TestCase):
    ...
    def test_fastapi_query_parser_middleware_url_encoded_nested_query(self):
        ...
        response = self.client.get('/search?query=authors%3E(authors.surname:Strindberg+~+(NOT+authors.type:editor))')
        expected = {
            'nested': {
                'path': 'authors',
                'query': {
    ...
    def test_use_fastapi_query_parser_requires_request_kwarg(self):
        ...
        self.assertIn("requires the decorated function to accept a FastAPI Request", str(cm.exception))
